...
export function NuevaAsignacion({ pacientes, estudiantes, docentes, token, onSuccess }: Props) {
  const { playSound } = useSoundPlayer() // Inicializamos el reproductor
  const [abierto, setAbierto] = useState(false)
  const [cargando, setCargando] = useState(false)
  const [formData, setFormData] = useState({ pacienteId: "", estudianteId: "", docenteId: "" })

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.pacienteId || !formData.estudianteId || !formData.docenteId) {
      alert("Por favor, selecciona paciente, estudiante y docente.")
      return
    }
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/pacientes/${formData.pacienteId}/`, {
        ...
        body: JSON.stringify({
          estudiante_asignado: parseInt(formData.estudianteId),
          docente_asignado: parseInt(formData.docenteId)
        }),
      })
      ...
      <DialogContent className="sm:max-w-[400px]"> 
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid gap-4 py-6">
            ...
            <div className="grid gap-2">
              <Label htmlFor="docente" className="text-sm font-semibold">Docente</Label>
              <select 
                id="docente"
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
