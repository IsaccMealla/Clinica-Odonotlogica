...
import { useClinicaWebSocket } from "@/hooks/useClinicaWebSocket"
...
export default function DetalleTratamientoPage() {
  ...
  const handleRealtimeMessage = useCallback((message: any) => {
    ...
    const currentId = tratamientoId?.toString()
    const targetTreatmentId = message.data?.tratamiento_id?.toString()

    if (model === 'tratamiento' && message.id === currentId) {
      fetchDatosTratamiento()
      return
    }

    if (model === 'avanceclinico' && targetTreatmentId === currentId) {
      fetchDatosTratamiento()
      return
    }
  }, [tratamientoId, fetchDatosTratamiento])

  const { connected } = useClinicaWebSocket(handleRealtimeMessage)
